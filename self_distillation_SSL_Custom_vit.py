...
labeled_dataloader = load_octmnist_data(BATCH_SIZE, split="val")

# Token Reduction Mechanism
class LearnableTokenSelector(nn.Module):
    def __init__(self, embed_dim, token_keep_ratio):
        super().__init__()
    ...
    def forward(self, x):
        scores = self.selector(x).squeeze(-1)  # (B, N)
        num_tokens_to_keep = max(1, int(self.token_keep_ratio * x.shape[1]))
        selected_tokens = torch.topk(scores, k=num_tokens_to_keep, dim=1)[1]
        return x.gather(1, selected_tokens.unsqueeze(-1).expand(-1, -1, x.shape[2]))

# Custom Transformer Model
class CustomTransformer(nn.Module):
    def __init__(self, img_size=256, patch_size=32, in_chans=3, num_classes=0, embed_dim=128, depth=6,
                 num_heads=4, mlp_ratio=4., attention_types=['full', 'linear', 'local'], token_keep_ratio=TOK_RATIO):
        super().__init__()
        self.patch_embed = nn.Conv2d(in_chans, embed_dim, kernel_size=patch_size, stride=patch_size)
        num_patches = (img_size // patch_size) ** 2
        self.pos_embed = nn.Parameter(torch.zeros(1, num_patches, embed_dim))
        
        # Layer Weighting Components
        ...
        self.token_selector = LearnableTokenSelector(embed_dim, token_keep_ratio)
...
class CustomTransformerBackbone(nn.Module):
    def __init__(self, token_keep_ratio=TOK_RATIO):
        super().__init__()
        self.transformer = CustomTransformer(img_size=256, patch_size=32, in_chans=3, embed_dim=128, depth=6,
...
def distillation_loss(student_logits, teacher_logits, temperature=2.0):
    """
    Compute the distillation loss (KL divergence between student and teacher outputs).
    """
...
def update_teacher_model(student_model, teacher_model, alpha=0.999):
    """
    Update the teacher model using an exponential moving average (EMA) of the student model's weights.
    """
...
def train_with_distillation(student_model, teacher_model, dataloader, optimizer, criterion, epochs, temperature=2.0, alpha=0.999):
    student_model.train()
    teacher_model.train()

    for epoch in range(epochs):
        epoch_loss = 0.0
        for i, (views, labels) in enumerate(tqdm(dataloader, desc=f"Epoch {epoch+1}/{epochs}")):
            view1, view2 = views[0].to(DEVICE), views[1].to(DEVICE)
            labels = labels.to(DEVICE)

            # Forward pass through student and teacher models
            online_pred_feat, target_proj_feat = student_model(view1, view2)
            with torch.no_grad():
                _, teacher_proj_feat = teacher_model(view1, view2)

            # Compute standard loss (e.g., cosine similarity)
            standard_loss = 1 - torch.mean(criterion(online_pred_feat, target_proj_feat))

            # Compute distillation loss
            dist_loss = distillation_loss(online_pred_feat, teacher_proj_feat, temperature)

            # Combine losses
            loss = standard_loss + 0.5 * dist_loss  # Adjust weights as needed
...
student_model = DualStreamNetwork().to(DEVICE)
teacher_model = DualStreamNetwork().to(DEVICE)
...
flops = FlopCountAnalysis(student_model.online_network[0], dummy_input)
print(f"Model FLOPs: {flops.total() / 1e9:.4f} GFLOPs")


# Measue Inference Latency Before Training
measure_inference_latency(student_model, dummy_input)


# Train with distillation
train_with_distillation(student_model, teacher_model, unlabeled_dataloader, optimizer, criterion, EPOCHS)
# Compute FLOPs after training
flops_after_training = FlopCountAnalysis(student_model.module.online_network[0], dummy_input)
print(f"Model FLOPs after training: {flops_after_training.total() / 1e9:.4f} GFLOPs")

# Save the trained model
torch.save(student_model.state_dict(), PRETRAINED_MODEL_PATH)